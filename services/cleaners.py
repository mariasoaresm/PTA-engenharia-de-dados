...
class DataCleaner:
    ...
    def clean_orders(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Conversão Vetorizada de Datas para UTC
        for col in date_cols:
            if col in df.columns:
                df[col] = pd.to_datetime(df[col], errors='coerce', utc=True)

        # Regra de Negócio: Remover pedidos sem timestamp de compra (lixo de dados)
    ...
    def clean_generic(df: pd.DataFrame) -> pd.DataFrame:
        """Fallback para entidades sem regras específicas."""
