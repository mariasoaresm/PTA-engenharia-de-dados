...
# função auxiliar para identificar registros órfãos
def get_orphan_mask(df_order_items, df_orders, df_products, df_sellers):
    """Calcula a máscara booleana para identificar registros órfãos."""
...
# 2. função principal de processamento com logging e validação
def process_payload_with_logging(payload, max_rows_limit=10000, request_user="API_Caller"):

    request_id = str(uuid.uuid4())
    start_time = time.time()
    
    # dicionário para armazenar métricas e auditoria
    audit_metrics = {
    ...
    for dataset_name, df in payload.items():
        ...
        # 2. tipagem (conversões e warnings)
        df_copy = df.copy()
    ...
    # fim do processamento e métricas finais
    end_time = time.time()
    ...
    logger.info(f"[{request_id}] Total de linhas recebidas: {total_rows}")
    logger.info(f"[{request_id}] Registros órfãos/descartados: {audit_metrics['discarded_count']}")
    logger.info(f"[{request_id}] Métricas de auditoria completas: {audit_metrics}")
    logger.info(request_id)
